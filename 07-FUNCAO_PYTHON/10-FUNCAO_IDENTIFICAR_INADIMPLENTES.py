...
def cliente_devedor(lista_devedores):
    lista_inadimplentes = []

    for devedores in lista_devedores:
        cpf, valor, dias = devedores
        if valor > 1000 and dias > 20:
            lista_inadimplentes.append((cpf,valor,dias))
    return lista_inadimplentes


resultado = cliente_devedor(clientes_devedores)
if resultado:
    resultado_formatado = '\n'.join([f'CPF: {cpf}, Valor devido: R${valor}, Dias de atraso: {dias}' for (cpf, valor, dias) in resultado])
    print(f'Os clientes devedores possuem os seguintes CPFs, valores devidos e dias de atraso:\n{resultado_formatado}')
else:
    print('Nenhum cliente devedor encontrado.')

print('Quantidade de clientes inamdiplente',len(resultado))
