...
def separa_lista(preco, tamanho, fator=0.1):
    if len(preco) == len(tamanho):
        i = int((1 - fator) * len(preco))
        precos_imoveis_treino = preco[:i]
        precos_imoveis_teste = preco[i:]
...
preco_treino, preco_teste, tamanho_treino, tamanho_teste = separa_lista(precos_imoveis, tamanho_imoveis)
